...
private def frameHeaderRoundtrip : List TestResult :=
  ...
  | some decoded =>
    ...
    , check "Frame header roundtrip stream" (decoded.streamId.val == 7)
...
private def goawayRoundtrip : List TestResult :=
  let frame := buildGoawayFrame (mkSid 5) .noError
  match decodeGoaway frame.payload with
  | some (lastStream, errCode, debugData) =>
    [ check "GOAWAY lastStream" (lastStream.val == 5)
...
private def windowUpdateRoundtrip : List TestResult :=
  let frame := buildWindowUpdateFrame (mkSid 3) 1000
...
private def rstStreamRoundtrip : List TestResult :=
  let frame := buildRstStreamFrame (mkSid 3) .cancel
...
def tests : List TestResult :=
  ...
  , let pingData := ByteArray.empty |>.push 1 |>.push 2 |>.push 3 |>.push 4
  ...
  , check "Priority encode size" ((encodePriority true (mkSid 5) 200).size == 5)
  , let payload := ByteArray.empty.push 0xAA |>.push 0xBB |>.push 0xCC
    check "Padding total size" ((encodePadding payload 5).size == 9)
  , check "Validate PING size=8 ok"
      (validateFrameSize { payloadLength := 8, frameType := .ping, flags := 0, streamId := StreamId.zero } Settings.default == none)
  , check "Validate PING size=7 error"
      (validateFrameSize { payloadLength := 7, frameType := .ping, flags := 0, streamId := StreamId.zero } Settings.default == some .frameSizeError)
  , check "Validate RST_STREAM size=4 ok"
      (validateFrameSize { payloadLength := 4, frameType := .rstStream, flags := 0, streamId := mkSid 1 } Settings.default == none)
  , check "Validate RST_STREAM size=3 error"
      (validateFrameSize { payloadLength := 3, frameType := .rstStream, flags := 0, streamId := mkSid 1 } Settings.default == some .frameSizeError)
  , check "Validate PRIORITY size=5 ok"
      (validateFrameSize { payloadLength := 5, frameType := .priority, flags := 0, streamId := mkSid 1 } Settings.default == none)
  , check "Validate PRIORITY size=6 error"
      (validateFrameSize { payloadLength := 6, frameType := .priority, flags := 0, streamId := mkSid 1 } Settings.default == some .frameSizeError)
  , check "Validate SETTINGS size=12 ok"
      (validateFrameSize { payloadLength := 12, frameType := .settings, flags := 0, streamId := StreamId.zero } Settings.default == none)
  , check "Validate SETTINGS size=7 error"
      (validateFrameSize { payloadLength := 7, frameType := .settings, flags := 0, streamId := StreamId.zero } Settings.default == some .frameSizeError)
  , check "Validate SETTINGS ACK size=0 ok"
      (validateFrameSize { payloadLength := 0, frameType := .settings, flags := FrameFlags.ack, streamId := StreamId.zero } Settings.default == none)
  , check "Validate SETTINGS ACK size=6 error"
      (validateFrameSize { payloadLength := 6, frameType := .settings, flags := FrameFlags.ack, streamId := StreamId.zero } Settings.default == some .frameSizeError)
  , check "Validate WINDOW_UPDATE size=4 ok"
      (validateFrameSize { payloadLength := 4, frameType := .windowUpdate, flags := 0, streamId := StreamId.zero } Settings.default == none)
  , check "Validate exceeds maxFrameSize"
      (validateFrameSize { payloadLength := 16385, frameType := .data, flags := 0, streamId := mkSid 1 } Settings.default == some .frameSizeError)
  , let block := ByteArray.empty |>.push 1 |>.push 2 |>.push 3 |>.push 4 |>.push 5
  ...
  , proofCovered "StreamId.hBit (31-bit constraint)" "StreamId.hBit"
  ] ++ frameHeaderRoundtrip ++ settingsRoundtrip ++ goawayRoundtrip
    ++ windowUpdateRoundtrip ++ rstStreamRoundtrip
    ++ priorityRoundtrip1 ++ priorityRoundtrip2
    ++ paddingRoundtrip ++ paddingZero ++ integerRoundtrip
