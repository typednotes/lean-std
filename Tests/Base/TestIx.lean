...
def tests : List TestResult :=
  ...
  -- Nat index (now returns Option {n : Nat // n < rangeSize bounds})
  , checkEq "Ix Nat index (3,7) 5" (some 2) ((Ix.index (3, 7) 5).map (·.val))
  , checkEq "Ix Nat index (3,7) 3" (some 0) ((Ix.index (3, 7) 3).map (·.val))
  , checkEq "Ix Nat index (3,7) 7" (some 4) ((Ix.index (3, 7) 7).map (·.val))
  , checkEq "Ix Nat index (3,7) 8 out" (none : Option Nat) ((Ix.index (3, 7) (8 : Nat)).map (·.val))
  , checkEq "Ix Nat index (3,7) 2 out" (none : Option Nat) ((Ix.index (3, 7) (2 : Nat)).map (·.val))
  -- Nat inRange
  ...
  -- Char index
  , checkEq "Ix Char index ('a','f') 'c'" (some 2) ((Ix.index ('a', 'f') 'c').map (·.val))
  , checkEq "Ix Char index ('a','f') 'z' out" (none : Option Nat) ((Ix.index ('a', 'f') 'z').map (·.val))
  -- Char inRange
  ...
  -- Bool index
  , checkEq "Ix Bool index (false,true) true" (some 1) ((Ix.index (false, true) true).map (·.val))
  , checkEq "Ix Bool index (false,true) false" (some 0) ((Ix.index (false, true) false).map (·.val))
  -- Int range
  , checkEq "Ix Int range (-2,2)" ([-2, -1, 0, 1, 2] : List Int) (Ix.range ((-2 : Int), 2))
  , checkEq "Ix Int index (-2,2) 0" (some 2) ((Ix.index ((-2 : Int), (2 : Int)) (0 : Int)).map (·.val))
  -- Proof coverage
